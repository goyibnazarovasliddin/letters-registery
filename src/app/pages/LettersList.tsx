...
export function LettersList() {
    ...
    const fetchLetters = async (silent = false) => {
        ...
        try {
            const res = await api.letters.list({ q: search, status: statusFilter, page, limit: 10 });
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4">
                <form onSubmit={handleSearch} className="flex gap-2">
                    <div className="relative flex-1">
                        ...
                        <Input
                            placeholder="Qidirish... (raqam, mavzu, qabul qiluvchi)"
                            value={search}
                            ...
                            <TableBody>
                                {letters.length === 0 ? (
                                    ...
                                    letters.map((letter, index) => (
                                        ...
                                        >
                                            <TableCell className="text-center text-gray-500">{index + 1 + (page - 1) * 10}</TableCell>
                                            <TableCell className="font-medium flex items-center gap-2 dark:text-gray-200">
                                                <FileText className="w-4 h-4 text-green-500" />
                                                {letter.letterNumber}
                                            </TableCell>
                                            <TableCell className="dark:text-gray-300">{letter.letterDate}</TableCell>
