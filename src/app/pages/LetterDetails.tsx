...
import { ChevronLeft, Calendar, FileText, User, Download, File, Send, LayoutDashboard, List } from 'lucide-react';
    ...
    return (
        <div className="space-y-6 max-w-4xl mx-auto animate-in fade-in duration-500 pb-10">
            ...
            <div className="flex items-center text-sm text-gray-500 dark:text-gray-400 mb-2">
                ...
                <span className="truncate max-w-[200px]">{letter.letterNumber || 'Xat'}</span>
            </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-3">
                            <h2 className="text-2xl font-bold dark:text-gray-100">{letter.letterNumber}</h2>
                        ...
                        <p className="text-gray-500 text-sm mt-1">Yaratildi: {new Date(letter.createdDate).toLocaleString()}</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="md:col-span-2 space-y-6">
                    <Card>
                        <CardContent className="p-6 space-y-6">
                            <div>
                                ...
                                <p className="text-lg font-medium">{letter.subject}</p>
                            </div>

                            <div className="grid grid-cols-2 gap-6">
                                <div>
                                    <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-1">Qabul qiluvchi</h3>
                                    <p className="flex items-center gap-2"><BuildingIcon className="w-4 h-4 text-gray-400" /> {letter.recipient}</p>
                                </div>
                                <div>
                                    <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-1">Sana</h3>
                                    <p className="flex items-center gap-2"><Calendar className="w-4 h-4 text-gray-400" /> {letter.letterDate}</p>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <p className="text-gray-700 bg-gray-50 p-4 rounded-lg">
                                    {letter.content || "Mazmun kiritilmagan"}
                                </p>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    {/* Files Section */}
                    <h3 className="font-semibold text-lg px-1">Biriktirilgan fayllar</h3>
                        ...
                        {letter.files.xat && (
                            <div className="bg-white border rounded-lg p-4 flex items-center justify-between shadow-sm">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="font-medium text-sm">{letter.files.xat.fileName}</p>
                                        <p className="text-xs text-gray-500">Asosiy xat • {(letter.files.xat.size / 1024 / 1024).toFixed(2)} MB</p>
                                    </div>
                                </div>
                                <Button size="sm" variant="outline" onClick={() => api.files.download(letter.files.xat!.id)}>
                            ...
                            <div key={file.id} className="bg-white border rounded-lg p-4 flex items-center justify-between shadow-sm">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500">Ilova • {(file.size / 1024 / 1024).toFixed(2)} MB</p>
                                    </div>
                                </div>
                                <Button size="sm" variant="outline" onClick={() => api.files.download(file.id)}>
                                    <Download className="w-4 h-4 mr-2" /> Yuklab olish
                                </Button>
                            </div>
                        ))}
                        ...
                        <CardContent className="p-6 space-y-4">
                            ...
                            <div className="space-y-3 text-sm">
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-medium">{letter.indexCode} - {letter.indexName}</span>
                                </div>
                                <div className="flex justify-between">
