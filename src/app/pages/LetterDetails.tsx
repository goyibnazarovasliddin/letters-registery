...
export function LetterDetails() {
    ...
    const handleRegister = async () => {
        ...
        if (!letter.indexCode && !letter.indexName) {
            toast.error(t('toast.registerIndexRequired'));
            return;
        }
        if (!letter.recipient) {
            toast.error(t('toast.registerRecipientRequired'));
            return;
        }
        if (!letter.subject) {
            toast.error(t('toast.registerSubjectRequired'));
            return;
        }
            ...
            <div className="flex items-center text-sm text-gray-500 dark:text-gray-400 mb-2">
                {from === 'dashboard' ? (
                    ...
                    <>
                        ...
                        <span>{t('lettersList.title')}</span>
                    </>
                )}
                <span className="mx-2">/</span>
                <span className="truncate max-w-[200px]">{t('letter.breadcrumb')}</span>
            </div>
                    ...
                    <div>
                        <div className="flex flex-col gap-1">
                            ...
                            <p className="text-sm font-medium text-gray-600 dark:text-gray-400">
                                {t('letter.registeredAt')}: {formatDateTime(letter.status === 'REGISTERED' ? (letter.registeredAt || letter.updatedDate) : letter.updatedDate)}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="md:col-span-2 space-y-6">
                    <Card>
                        <CardContent className="p-6 space-y-6">
                            <div>
                                <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-1">{t('letter.subject')}</h3>
                                <p className="text-lg font-medium">{letter.subject || <span className="text-gray-400 italic font-normal">{t('letter.emptyField')}</span>}</p>
                            </div>

                            <div className="grid grid-cols-2 gap-6">
                                <div>
                                    <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-1">{t('letter.recipient')}</h3>
                                    <p className="flex items-center gap-2"><BuildingIcon className="w-4 h-4 text-gray-400" /> {letter.recipient || <span className="text-gray-400 italic">{t('letter.emptyField')}</span>}</p>
                                </div>
                                <div>
                                    <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-1">{t('letter.date')}</h3>
                            ...
                            <div>
                                <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-1">{t('letter.summary')}</h3>
                                <p className="text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                                    {letter.summary || t('letter.summaryEmpty')}
                                </p>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <div className="flex items-center justify-between px-1">
                        <h3 className="font-semibold text-lg">{t('letter.attachedFiles')}</h3>
                    </div>
                    <div className="space-y-3">
                        {!letter.files.xat && (!letter.files.ilova || letter.files.ilova.length === 0) ? (
                            <div className="bg-gray-50 dark:bg-gray-800/50 border border-dashed rounded-lg p-8 text-center">
                                ...
                                <p className="text-gray-500 dark:text-gray-400 text-sm italic">{t('letter.filesNone')}</p>
                            </div>
                        ) : (
                            <>
                                {letter.files.xat && (
                                    <div className="bg-white dark:bg-gray-800/50 border dark:border-gray-700 rounded-lg p-4 flex items-center justify-between shadow-sm">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-gray-500 dark:text-gray-400">{t('letter.mainLetterShort')} • {(letter.files.xat.size / 1024 / 1024).toFixed(2)} MB</p>
                                            </div>
                                        </div>
                                        <Button size="sm" variant="outline" onClick={() => api.files.download(letter.files.xat!.id)} className="dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-700">
                                            <Download className="w-4 h-4 mr-2" /> {t('letter.download')}
                                        </Button>
                                    </div>
                                )}
                                    ...
                                    <div key={file.id} className="bg-white dark:bg-gray-800/50 border dark:border-gray-700 rounded-lg p-4 flex items-center justify-between shadow-sm">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-gray-500 dark:text-gray-400">{t('letter.attachment')} • {(file.size / 1024 / 1024).toFixed(2)} MB</p>
                                            </div>
                                        </div>
                ...
                <div className="space-y-6">
                    <Card>
                        <CardContent className="p-6 space-y-4">
                            <h3 className="font-medium border-b pb-2">{t('letter.executorInfo')}</h3>
                            <div className="flex items-center gap-3">
                    ...
                    <Card>
                        <CardContent className="p-6 space-y-4">
                            <h3 className="font-medium border-b pb-2">{t('letter.docParams')}</h3>
                            <div className="space-y-3 text-sm">
                                {letter.letterNumber && (
                                    <div className="flex justify-between items-center">
                                        <span className="text-gray-500">{t('letter.number')}:</span>
                                ...
                                <div className="flex justify-between">
                                    <span className="text-gray-500">{t('letter.index')}:</span>
                                    <span className="font-medium">{letter.indexCode ? `${letter.indexCode} - ${letter.indexName}` : <span className="text-gray-400 italic font-normal">{t('letter.emptyField')}</span>}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-gray-500">{t('letter.pageCount')}:</span>
                                    <span className="font-medium">{letter.pageCount}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-gray-500">{t('letter.attachmentPageCount')}:</span>
